locals {
  interactive_resource_group_name   = upper(format("%s-%s", var.customer_code, var.interactive_resource_group_name))
  ...
  tags                              = merge(module.interactive_standard_tags.interactive_standard_tags, var.additional_tags)
}
...
########## Create Interactive Resource Group ############################################
resource "azurerm_resource_group" "interactive_resource_group" {
  count    = var.create_interactive_resource_group ? 1 : 0
  name     = local.interactive_resource_group_name
  location = var.interactive_resource_group_location
  tags     = local.tags
}

data "azurerm_resource_group" "interactive_resource_group" {
  count = var.create_interactive_resource_group ? 0 : 1
...
resource "azurerm_availability_set" "management_vm_availability_set" {
  ...
  resource_group_name          = var.create_interactive_resource_group ? azurerm_resource_group.interactive_resource_group[0].name : data.azurerm_resource_group.interactive_resource_group[0].name
  location                     = var.create_interactive_resource_group ? azurerm_resource_group.interactive_resource_group[0].location : data.azurerm_resource_group.interactive_resource_group[0].location
  platform_fault_domain_count  = var.platform_fault_domain_count
  platform_update_domain_count = var.platform_update_domain_count
  tags                         = local.tags
  depends_on = [
    azurerm_resource_group.interactive_resource_group
  ]
}
...
resource "azurerm_network_interface" "management_vm_nic" {
  ...
  ip_configuration {
    name                          = "ipconfig1"
    subnet_id                     = var.interactive_subnet_id
...
resource "azurerm_windows_virtual_machine" "management_vm" {
  ...
  tags = merge(
    local.tags,
    module.interactive_standard_tags.interactive_virtual_machine_tags
  )
  depends_on = [
    azurerm_resource_group.interactive_resource_group
  ]
...
########## Interactive internal subnet NSG rules for management server  #################
resource "azurerm_network_security_rule" "Allow_RDP_to_Jumpbox" {
  ...
  network_security_group_name = var.interactive_subnet_nsg_name
}
